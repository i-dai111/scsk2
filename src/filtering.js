  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      Papa.parse(csvData, {
        ...
        complete: (results) => {
          ...
          setLoading(false); // データ読み込み完了後にfalseにする
          if (loading) {
    ...
    const filterData = () => {
      ...
      return data.filter((row) => {
        ...
        return (
          (answers.remoteWork === "いいえ" || answers.remoteWork === "" || row["リモートワーク"] === answers.remoteWork)  &&
    ...
    const callChatGPT = async (filteredCompanies, userCulturePreference) => {
      const prompt = `
        ...
        社風がユーザーの好みに合いそうな２つの会社を選んで、その理由を100文字程度で説明してください。
      `;
